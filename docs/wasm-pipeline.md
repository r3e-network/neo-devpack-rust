...
3. Enforce constraints that keep the first version tractable: single memory (unused), no indirect calls, no floating point, no reference types.
...
5. Events & manifests – infer event signatures from `neo_event` macros, auto-fill manifest `abi.events`.
6. Debug metadata – attach sequence points, contract hash hints, gas metering.

## 7. Developer workflow summary

1. `cargo build-wasm` (new alias) → produces `target/wasm32-unknown-unknown/release/<name>.wasm`.
2. `wasm-neovm translate --input <name>.wasm --out build/<name>.nef --manifest build/<name>.manifest.json`.
3. Optionally run `neo-cli` or emulator tests.
4. Package artifacts for deployment.

Automate steps 1–3 behind a single command (`cargo neovm build`).
