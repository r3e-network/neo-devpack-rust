...
```
-C panic=abort -C target-feature=-simd128,-reference-types,-multivalue,-tail-call
```

Any attempt to compile a contract that uses SIMD, reference types beyond `funcref`, tail calls, or multi-value returns fails during `cargo build` before the translator is invoked. Atomics remain unsupported by the translator. `scripts/build_contract.sh` inherits those defaults and adds `-C opt-level=z` / `-C strip=symbols`; the environment variable `NEO_WASM_RUSTFLAGS` can override the entire flag string when required (for example, to re-enable bulk-memory experiments).
