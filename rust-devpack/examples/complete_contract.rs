...
use neo_devpack::{neo_storage, neo_serialize, NeoStorageContext};
...
impl CompleteContract {
    ...
    pub fn new(name: NeoString, version: NeoString, total_supply: NeoInteger, decimals: NeoInteger) -> Self {
        Self {
            name,
            version,
    ...
    pub fn initialize(&mut self) -> NeoResult<()> {
        ...
        storage.metadata.insert(NeoString::from_str("name"), NeoValue::from(self.name.clone()));
        storage.metadata.insert(NeoString::from_str("version"), NeoValue::from(self.version.clone()));
        storage.metadata.insert(NeoString::from_str("total_supply"), NeoValue::from(self.total_supply));
        storage.metadata.insert(NeoString::from_str("decimals"), NeoValue::from(self.decimals));
        
        // Initialize counters
        storage.counters.insert(NeoString::from_str("transfer_count"), NeoInteger::ZERO);
        storage.counters.insert(NeoString::from_str("approval_count"), NeoInteger::ZERO);
        
        // Distribute initial supply to owner
        storage.balances.insert(self.owner.clone(), self.total_supply);
    ...
    pub fn balance_of(&self, account: &NeoByteString) -> NeoResult<NeoInteger> {
        ...
        Ok(storage.balances.get(account).cloned().unwrap_or(NeoInteger::ZERO))
    }
    ...
    pub fn transfer(&mut self, to: &NeoByteString, amount: NeoInteger) -> NeoResult<NeoBoolean> {
        ...
        // Add to receiver
        let to_balance = storage.balances.get(to).cloned().unwrap_or(NeoInteger::ZERO);
        let new_to_balance = to_balance + amount;
        storage.balances.insert(to.clone(), new_to_balance);
        
        // Update transfer counter
        let transfer_count = storage.counters.get(&NeoString::from_str("transfer_count"))
            .cloned()
            .unwrap_or(NeoInteger::ZERO);
        storage.counters.insert(NeoString::from_str("transfer_count"), transfer_count + NeoInteger::ONE);
    ...
    /// Approve spender
    #[neo_method]
    pub fn approve(&mut self, spender: &NeoByteString, amount: NeoInteger) -> NeoResult<NeoBoolean> {
        // Validate amount
        if amount < NeoInteger::ZERO {
            return Ok(NeoBoolean::FALSE);
        }
        ...
        let mut owner_allowances = storage.allowances.get(&owner)
            .cloned()
        ...
        let approval_count = storage.counters.get(&NeoString::from_str("approval_count"))
            .cloned()
            .unwrap_or(NeoInteger::ZERO);
        storage.counters.insert(NeoString::from_str("approval_count"), approval_count + NeoInteger::ONE);
    ...
    /// Get allowance
    #[neo_method]
    pub fn allowance(&self, owner: &NeoByteString, spender: &NeoByteString) -> NeoResult<NeoInteger> {
        let storage = CompleteStorage::load(&NeoRuntime::get_storage_context()?);
        
        if let Some(owner_allowances) = storage.allowances.get(owner) {
            Ok(owner_allowances.get(spender).cloned().unwrap_or(NeoInteger::ZERO))
        } else {
            Ok(NeoInteger::ZERO)
        }
    }
    ...
    #[neo_method]
    pub fn transfer_from(&mut self, from: &NeoByteString, to: &NeoByteString, amount: NeoInteger) -> NeoResult<NeoBoolean> {
        // Validate amount
        if amount <= NeoInteger::ZERO {
            return Ok(NeoBoolean::FALSE);
        }
        ...
        // Update allowance
        let mut owner_allowances = storage.allowances.get(from)
            .cloned()
            .unwrap_or(NeoMap::new());
    ...
    pub fn get_transfer_count(&self) -> NeoResult<NeoInteger> {
        let storage = CompleteStorage::load(&NeoRuntime::get_storage_context()?);
        Ok(storage.counters.get(&NeoString::from_str("transfer_count"))
            .cloned()
            .unwrap_or(NeoInteger::ZERO))
    }
    ...
    pub fn get_approval_count(&self) -> NeoResult<NeoInteger> {
        let storage = CompleteStorage::load(&NeoRuntime::get_storage_context()?);
        Ok(storage.counters.get(&NeoString::from_str("approval_count"))
            .cloned()
            .unwrap_or(NeoInteger::ZERO))
    }
    ...
    pub fn update_contract(&mut self, new_version: NeoString) -> NeoResult<NeoBoolean> {
        ...
        // Update storage
        let mut storage = CompleteStorage::load(&NeoRuntime::get_storage_context()?);
        storage.metadata.insert(NeoString::from_str("version"), NeoValue::from(new_version.clone()));
        storage.save(&NeoRuntime::get_storage_context()?)?;
    ...
    pub fn get_statistics(&self) -> NeoResult<NeoValue> {
        ...
        stats.set_field("transfer_count", NeoValue::from(
            storage.counters.get(&NeoString::from_str("transfer_count"))
                .cloned()
                .unwrap_or(NeoInteger::ZERO)
        ));
        stats.set_field("approval_count", NeoValue::from(
            storage.counters.get(&NeoString::from_str("approval_count"))
                .cloned()
                .unwrap_or(NeoInteger::ZERO)
        ));
        stats.set_field("balance_count", NeoValue::from(NeoInteger::new(storage.balances.len() as i32)));
        stats.set_field("allowance_count", NeoValue::from(NeoInteger::new(storage.allowances.len() as i32)));
    ...
    pub fn emergency_pause(&mut self) -> NeoResult<NeoBoolean> {
        ...
        // Set emergency flag
        let mut storage = CompleteStorage::load(&NeoRuntime::get_storage_context()?);
        storage.metadata.insert(NeoString::from_str("emergency_paused"), NeoValue::from(NeoBoolean::TRUE));
        storage.save(&NeoRuntime::get_storage_context()?)?;
    ...
    pub fn emergency_unpause(&mut self) -> NeoResult<NeoBoolean> {
        ...
        let mut storage = CompleteStorage::load(&NeoRuntime::get_storage_context()?);
        storage.metadata.insert(NeoString::from_str("emergency_paused"), NeoValue::from(NeoBoolean::FALSE));
        storage.save(&NeoRuntime::get_storage_context()?)?;
    ...
    pub fn is_paused(&self) -> NeoResult<NeoBoolean> {
        let storage = CompleteStorage::load(&NeoRuntime::get_storage_context()?);
        let paused = storage.metadata.get(&NeoString::from_str("emergency_paused"))
            .cloned()
...
pub fn deploy_contract() -> NeoResult<()> {
    ...
    // Log deployment
    NeoRuntime::log(&NeoString::from_str("CompleteContract deployed successfully"))?;
...
mod tests {
    ...
    fn test_complete_contract_operations() {
        ...
        // Test transfer from
        let transfer_from_result = contract.transfer_from(&account1, &account2, NeoInteger::new(25));
