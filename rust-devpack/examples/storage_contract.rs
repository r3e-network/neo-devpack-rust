...
impl StorageContract {
    ...
    pub fn initialize(&mut self) -> NeoResult<()> {
        ...
        storage.counters.insert(NeoString::from_str("user_count"), NeoInteger::ZERO);
    ...
    /// Register a new user
    #[neo_method]
    pub fn register_user(&mut self, user_id: &NeoByteString, user_data: UserData) -> NeoResult<NeoBoolean> {
        ...
        // Update user count
        let current_count = storage.counters.get(&NeoString::from_str("user_count"))
            .cloned()
            .unwrap_or(NeoInteger::ZERO);
        let new_count = current_count + NeoInteger::ONE;
        storage.counters.insert(NeoString::from_str("user_count"), new_count);
    ...
    /// Add a new user
    #[neo_method]
    pub fn add_user(&mut self, user_id: &NeoByteString, name: NeoString, age: NeoInteger, email: NeoString) -> NeoResult<NeoBoolean> {
        // Check if user already exists
        let mut storage = ContractStorage::load(&NeoRuntime::get_storage_context()?);
        if storage.users.get(user_id).is_some() {
            return Ok(NeoBoolean::FALSE);
    ...
    /// Update user balance
    #[neo_method]
    pub fn update_user_balance(&mut self, user_id: &NeoByteString, new_balance: NeoInteger) -> NeoResult<NeoBoolean> {
        let mut storage = ContractStorage::load(&NeoRuntime::get_storage_context()?);
    ...
    pub fn get_user_count(&self) -> NeoResult<NeoInteger> {
        let storage = ContractStorage::load(&NeoRuntime::get_storage_context()?);
        Ok(storage.counters.get(&NeoString::from_str("user_count"))
            .cloned()
            .unwrap_or(NeoInteger::ZERO))
    }
    ...
    pub fn increment_counter(&mut self, counter_name: NeoString) -> NeoResult<NeoInteger> {
        ...
        let current_value = storage.counters.get(&counter_name)
            .cloned()
            .unwrap_or(NeoInteger::ZERO);
    ...
    pub fn get_counter(&self, counter_name: &NeoString) -> NeoResult<NeoInteger> {
        let storage = ContractStorage::load(&NeoRuntime::get_storage_context()?);
        Ok(storage.counters.get(counter_name)
            .cloned()
            .unwrap_or(NeoInteger::ZERO))
    }
...
mod tests {
    ...
    fn test_settings_and_counters() {
        ...
        contract.set_setting(setting_key.clone(), setting_value).unwrap();
...
pub fn main() -> NeoResult<()> {
    ...
    let result = contract.register_user(
        &NeoByteString::from_slice(b"user1"),
        user_data.clone()
    )?;
