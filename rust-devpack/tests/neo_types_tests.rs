...
fn test_neo_map() {
    ...
    assert!(map.get(&NeoValue::from(NeoString::from_str("nonexistent"))).is_none());
...
fn test_neo_value() {
    ...
    // Test Map value
    let mut map = NeoMap::new();
    map.insert(NeoValue::from(NeoString::from_str("key")), NeoValue::from(NeoInteger::new(42)));
    ...
    // Test Struct value
    let struct_data = NeoStruct::new().with_field("name", NeoValue::from(NeoString::from_str("Test")));
...
fn test_neo_storage_context() {
    ...
    let read_only_context = NeoStorageContext::new(1);
...
fn test_neo_contract_trait() {
    ...
    impl NeoContract for TestContract {
        fn name() -> &'static str { "TestContract" }
        fn version() -> &'static str { "1.0.0" }
        fn author() -> &'static str { "Test Author" }
        fn description() -> &'static str { "Test contract" }
    }
...
fn test_neo_contract_entry_trait() {
    ...
    impl NeoContractEntry for TestContract {
        fn deploy() -> NeoResult<()> { Ok(()) }
        fn update() -> NeoResult<()> { Ok(()) }
        fn destroy() -> NeoResult<()> { Ok(()) }
    }
...
fn test_neo_contract_method_trait() {
    ...
    impl NeoContractMethod for TestMethod {
        fn name() -> &'static str { "test_method" }
        fn parameters() -> &'static [&'static str] { &["Integer", "String"] }
        fn return_type() -> &'static str { "Boolean" }
